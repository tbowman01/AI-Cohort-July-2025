...
async def test_story_generator():
    ...
    for i, test_case in enumerate(test_cases, 1):
        ...
        try:
            ...
            is_valid, issues = generator.validate_gherkin_syntax(result['gherkin_content'])
            
            print(f"✅ Story ID: {result['story_id']}")
            print(f"✅ Feature Type: {result['feature_type']} (expected: {test_case['expected_type']})")
...
async def test_integration():
    ...
    try:
        ...
        is_valid, issues = generator.validate_gherkin_syntax(refined_story['gherkin_content'])
...
async def main():
    ...
    test_results.append(('Story Generator', len([r for r in generator_results if r['success']]), len(generator_results)))
