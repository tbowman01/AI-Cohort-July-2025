...
from models import UserStory, Session as SessionModel, search_user_stories, get_user_story_stats
...
class TestUserStoryModel:
    ...
    def test_user_story_metadata_edge_cases(self, db_session):
        ...
        story = UserStory(
            feature_description="Test story",
    ...
    def test_user_story_to_dict(self, db_session):
        ...
        created_dt = datetime.fromisoformat(story_dict["created_at"].replace('Z', '+00:00'))
        assert isinstance(created_dt, datetime)

    def test_user_story_timestamps(self, db_session):
        """Test UserStory timestamp behavior."""
        story = UserStory(
            feature_description="Test timestamps",
            gherkin_output="Feature: Timestamps"
        )
    ...
    def test_user_story_large_content(self, db_session):
        ...
        story = UserStory(
            feature_description=large_description,
            gherkin_output=large_gherkin
        )
...
class TestSessionModel:
    ...
    def test_session_to_dict(self, db_session):
        ...
        # Verify datetime ISO format
        created_dt = datetime.fromisoformat(session_dict["created_at"].replace('Z', '+00:00'))
        assert isinstance(created_dt, datetime)
...
class TestModelUtilityFunctions:
    ...
    def test_get_user_story_stats(self, db_session):
        ...
        stories_data = [
            ...
            ("Medium length description for testing", "Feature: Medium\nScenario: Test scenario\nGiven test\nWhen test\nThen test"),
            ("Very long description with lots of details about the feature requirements", "Feature: Long\nScenario: Complex test\nGiven complex setup\nWhen multiple actions\nThen comprehensive verification"),
        ]
        ...
        assert stats["avg_feature_description_length"] < 1000  # Should be reasonable
        assert stats["avg_gherkin_output_length"] > 10  # Should have some content
...
class TestModelValidation:
    ...
    def test_user_story_field_lengths(self, db_session):
        ...
        story = UserStory(
            feature_description=very_long_description,
            gherkin_output=very_long_gherkin
        )
    ...
    def test_metadata_json_validation(self, db_session):
        ...
        story = UserStory(
            feature_description="JSON test",
            gherkin_output="Feature: JSON test"
        )
