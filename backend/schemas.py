...
class UserStoryBase(BaseModel):
    ...
    feature_description: str = Field(
        ...
        examples=[
            (
                "As a user, I want to be able to login to my account so that I "
...
class UserStoryUpdate(BaseModel):
    ...
    feature_description: Optional[str] = Field(
        None, min_length=10, max_length=5000, description="Updated feature description"
    )
...
class SessionBase(BaseModel):
    ...
    user_id: Optional[str] = Field(
        None, max_length=255, description="User identifier for session tracking"
    )
...
class DatabaseStats(BaseModel):
    ...
    oldest_story: Optional[datetime] = Field(
        None, description="Timestamp of oldest story"
    )
    newest_story: Optional[datetime] = Field(
        None, description="Timestamp of newest story"
    )
    avg_feature_description_length: float = Field(
        ..., description="Average length of feature descriptions"
    )
    avg_gherkin_output_length: float = Field(
        ..., description="Average length of Gherkin outputs"
    )
...
class HealthCheck(BaseModel):
    ...
    status: str = Field(
        ..., description="Service status", examples=["healthy", "unhealthy"]
    )
...
class BulkUserStoryCreate(BaseModel):
    ...
    stories: list[UserStoryCreate] = Field(
        ..., min_items=1, max_items=50, description="List of user stories to create"
    )
...
class BulkUserStoryResponse(BaseModel):
    ...
    created: list[UserStoryResponse] = Field(
        ..., description="Successfully created user stories"
    )
    failed: list[Dict[str, Any]] = Field(
        ..., description="Failed user story creations with errors"
    )
    total_requested: int = Field(..., description="Total number of stories requested")
    total_created: int = Field(
        ..., description="Total number of stories successfully created"
    )
