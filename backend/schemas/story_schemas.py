...
class AcceptanceCriteria(BaseModel):
    ...
    id: Optional[str] = Field(default=None, description="Unique identifier for the criterion")
    description: str = Field(..., min_length=5, description="Description of the acceptance criterion")
    priority: Priority = Field(default=Priority.MEDIUM, description="Priority of this criterion")
    testable: bool = Field(default=True, description="Whether this criterion is testable")
...
class StoryComponents(BaseModel):
    ...
    feature_type: Optional[str] = Field(default=None, description="Detected feature type")
...
class QualityMetrics(BaseModel):
    ...
    is_valid_gherkin: bool = Field(..., description="Whether the Gherkin syntax is valid")
    syntax_issues: List[str] = Field(default_factory=list, description="List of syntax issues found")
    ...
    completeness: Dict[str, bool] = Field(default_factory=dict, description="Completeness metrics")
...
class StoryResponse(BaseModel):
    ...
    estimated_effort: int = Field(ge=1, le=13, description="Estimated effort in story points")
    story_type: StoryType = Field(..., description="Type of story")
    priority: Priority = Field(..., description="Priority level")
    status: StoryStatus = Field(default=StoryStatus.DRAFT, description="Current status of the story")
    
    # Generation metadata
    feature_type: Optional[str] = Field(default=None, description="Detected feature type")
    generated_at: datetime = Field(default_factory=datetime.utcnow, description="When the story was generated")
    components: Optional[StoryComponents] = Field(default=None, description="Extracted story components")
    
    # AI metadata
    ai_provider: Optional[str] = Field(default=None, description="AI provider used for generation")
    ai_generated: bool = Field(default=False, description="Whether AI was used for generation")
    template_based: bool = Field(default=True, description="Whether template-based generation was used")
    confidence_score: Optional[float] = Field(default=None, ge=0.0, le=1.0, description="Confidence in the generation")
    
    # Quality metrics
    quality_metrics: Optional[QualityMetrics] = Field(default=None, description="Quality analysis of the story")
    ...
    # Refinement data
    refinement_feedback: Optional[str] = Field(default=None, description="Feedback used for refinement")
    refined_at: Optional[datetime] = Field(default=None, description="When the story was last refined")
...
class StoryListResponse(BaseModel):
    ...
    stories: List[StoryResponse] = Field(default_factory=list, description="List of stories")
...
class StoryValidationResponse(BaseModel):
    ...
    quality_metrics: QualityMetrics = Field(..., description="Quality analysis of the content")
...
class StorySuggestionsResponse(BaseModel):
    ...
    analyzed_description: str = Field(..., description="The analyzed feature description")
...
class AIProviderStatus(BaseModel):
    ...
    available_providers: List[str] = Field(default_factory=list, description="List of available providers")
    claude_configured: bool = Field(default=False, description="Whether Claude API is configured")
    openai_configured: bool = Field(default=False, description="Whether OpenAI API is configured")
    fallback_enabled: bool = Field(default=True, description="Whether template fallback is enabled")
...
class HealthCheckResponse(BaseModel):
    ...
    ai_provider_status: Optional[AIProviderStatus] = Field(default=None, description="AI provider status")
