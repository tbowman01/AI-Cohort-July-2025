...
class Settings(BaseSettings):
    ...
    api_host: str = Field(
        default="localhost", description="Host for the FastAPI application"
    )
    api_port: str = Field(
        default="8000", description="Port for the FastAPI application"
    )

    algorithm: str = Field(
        default="gpt-4", description="AI algorithm to use for story generation"
    )
    ...
    phase1_status: str = Field(
        default="completed", description="Phase 1 status of the project"
    )
    phase1_completion_date: str = Field(
        default="2025-07-31", description="Expected completion date for Phase 1"
    )
    ...
    app_description: str = Field(
        default="AI-Powered DevOps Tracker", description="Application description"
    )
    ...
    database_file: str = Field(
        default="autodevhub.db", description="SQLite database file path"
    )
    debug: bool = Field(
        default=False, description="Enable debug mode (shows SQL queries)"
    )
    create_sample_data: bool = Field(
        default=False, description="Create sample data on initialization"
    )
    ...
    max_db_connections: int = Field(
        default=20, ge=1, le=100, description="Maximum concurrent database connections"
    )
    ...
    openai_api_key: Optional[str] = Field(
        default=None, description="OpenAI API key for AI story generation"
    )
    ai_model: str = Field(
        default="gpt-4", description="AI model to use for story generation"
    )
    ai_max_tokens: int = Field(
        default=2000, ge=100, le=8000, description="Maximum tokens for AI generation"
    )

    # Security Settings (Future Enhancement)
    secret_key: Optional[str] = Field(
        default=None, description="Secret key for JWT token signing"
    )
    ...
    database_echo: bool = Field(
        default=False, description="Echo SQL queries (for development)"
    )
