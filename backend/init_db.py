...
This script creates the SQLite database, tables, indexes, and full-text search capabilities
as specified in ADR-003: Database Selection (SQLite).
"""
...
from models import UserStory, Session
...
async def create_sample_data():
    ...
    sample_stories = [
        {
            "feature_description": "As a user, I want to be able to login to my account so that I can access my personal dashboard",
  ...
  Scenario: Failed login with invalid credentials
    ...
    And I should remain on the login page""",
        ...
        {
            "feature_description": "As a project manager, I want to create user stories so that I can plan development sprints",
  ...
  Scenario: Edit an existing user story
    ...
    And the timestamp should reflect the change""",
        ...
        {
            "feature_description": "As an API user, I want to search through user stories so that I can find relevant examples",
...
async def main():
    ...
    try:
        ...
        if "--sample-data" in sys.argv or os.getenv("CREATE_SAMPLE_DATA", "false").lower() == "true":
...
Environment Variables:
    DATABASE_FILE           Path to SQLite database file (default: autodevhub.db)
    CREATE_SAMPLE_DATA      Set to 'true' to create sample data (default: false)
    DEBUG                   Set to 'true' to enable SQL query logging (default: false)

Examples:
    python init_db.py                    # Initialize database only
    python init_db.py --sample-data      # Initialize database with sample data
